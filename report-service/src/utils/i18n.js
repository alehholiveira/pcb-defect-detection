export const emailTranslations = {
  'pt-BR': {
    subjectPrefix: 'Relatório de Inspeção PCB',
    badgeText: 'Inspeção PCB',
    title: 'Relatório de Defeitos PCB',
    periodLabel: 'Período',
    summaryTitle: 'Resumo da Análise',
    totalInferences: 'Total de Inferências',
    totalImages: 'Imagens Analisadas',
    totalDefects: 'Defeitos Detectados',
    defectsBreakdown: 'Detalhamento dos Defeitos',
    downloadButton: 'Baixar Relatório em PowerPoint (PPTX)',
    footerNotice: 'Este é um e-mail automático gerado pelo Sistema de Detecção de Defeitos em PCB.',
    until: 'até',
    dailyReport: 'Relatório Diário',
    weeklyReport: 'Relatório Semanal',
    monthlyReport: 'Relatório Mensal',
    manualReport: 'Relatório Manual',
  },
  'en': {
    subjectPrefix: 'PCB Inspection Report',
    badgeText: 'PCB Inspection',
    title: 'PCB Defect Detection Report',
    periodLabel: 'Period',
    summaryTitle: 'Analysis Summary',
    totalInferences: 'Total Inferences',
    totalImages: 'Analyzed Images',
    totalDefects: 'Defects Detected',
    defectsBreakdown: 'Defects Breakdown',
    downloadButton: 'Download PowerPoint Report (PPTX)',
    footerNotice: 'This is an automated email generated by the PCB Defect Detection System.',
    until: 'to',
    dailyReport: 'Daily Report',
    weeklyReport: 'Weekly Report',
    monthlyReport: 'Monthly Report',
    manualReport: 'Manual Report',
  },
};

/**
 * Formats a date string (YYYY-MM-DD) according to the selected language.
 * @param {string} dateStr - Date string in YYYY-MM-DD format
 * @param {'pt-BR'|'en'} language - Selected language
 * @returns {string} Formatted date
 */
export function formatDate(dateStr, language = 'pt-BR') {
  if (!dateStr) return 'N/A';
  const parts = dateStr.split('-');
  if (parts.length !== 3) return dateStr;
  const [y, m, d] = parts;
  if (language === 'en') {
    return `${m}/${d}/${y}`;
  }
  return `${d}/${m}/${y}`;
}

/**
 * Formats a date range.
 * @param {string} oldest - Start date (YYYY-MM-DD)
 * @param {string} newest - End date (YYYY-MM-DD)
 * @param {'pt-BR'|'en'} language - Selected language
 * @returns {string} Formatted date range string
 */
export function formatDateRange(oldest, newest, language = 'pt-BR') {
  const t = emailTranslations[language] || emailTranslations['pt-BR'];
  const formattedOldest = formatDate(oldest, language);
  const formattedNewest = formatDate(newest, language);

  if (!oldest || !newest || oldest === newest) {
    return formattedOldest;
  }
  return `${formattedOldest} ${t.until} ${formattedNewest}`;
}

/**
 * Builds HTML email template styled consistently with the PCB Defect Detection design system.
 * @param {Object} params
 * @param {string} params.reportTitle
 * @param {string} params.periodLabel
 * @param {Object} params.stats - { totalInferences, totalImages, totalDefects }
 * @param {Object} [params.defectsByType] - Object with defect counts by class
 * @param {string} params.downloadUrl - Direct download link for PPTX
 * @param {'pt-BR'|'en'} [language='pt-BR'] - Language key
 * @returns {string} HTML string
 */
export function buildReportEmailHtml({ reportTitle, periodLabel, stats, defectsByType = {}, downloadUrl }, language = 'pt-BR') {
  const t = emailTranslations[language] || emailTranslations['pt-BR'];

  const defectsRows = defectsByType && Object.keys(defectsByType).length > 0
    ? Object.entries(defectsByType)
        .map(
          ([type, count]) => `
            <tr>
              <td style="padding: 8px 12px; border-bottom: 1px solid #E5E7EB; color: #6B7280;">${type}</td>
              <td style="padding: 8px 12px; border-bottom: 1px solid #E5E7EB; text-align: right; font-weight: 600; color: #111827;">${count}</td>
            </tr>`
        )
        .join('')
    : '';

  return `
<!DOCTYPE html>
<html lang="${language}">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${t.title}</title>
</head>
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #F9FAFB; color: #111827;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color: #F9FAFB; padding: 40px 20px;">
    <tr>
      <td align="center">
        <table role="presentation" width="100%" max-width="600" cellspacing="0" cellpadding="0" border="0" style="max-width: 600px; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -4px rgba(0, 0, 0, 0.1); border: 1px solid #E5E7EB;">
          
          <!-- Header Banner -->
          <tr>
            <td style="background: linear-gradient(135deg, #16A34A 0%, #15803D 100%); padding: 32px 32px; text-align: left;">
              <span style="background-color: rgba(255, 255, 255, 0.15); color: #F0FDF4; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; padding: 4px 12px; border-radius: 20px; border: 1px solid rgba(255, 255, 255, 0.3); display: inline-block; margin-bottom: 12px;">
                ${t.badgeText}
              </span>
              <h1 style="margin: 0; color: #ffffff; font-size: 24px; font-weight: 700; letter-spacing: -0.5px;">
                ${reportTitle || t.title}
              </h1>
              <p style="margin: 8px 0 0 0; color: #DCFCE7; font-size: 14px;">
                ${t.periodLabel}: <strong style="color: #ffffff;">${periodLabel}</strong>
              </p>
            </td>
          </tr>

          <!-- Content Body -->
          <tr>
            <td style="padding: 32px;">
              <h2 style="margin-top: 0; margin-bottom: 20px; color: #111827; font-size: 18px; font-weight: 600;">
                ${t.summaryTitle}
              </h2>

              <!-- Statistics Cards Grid -->
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-bottom: 24px;">
                <tr>
                  <td width="32%" style="background-color: #F9FAFB; border-radius: 12px; padding: 16px; text-align: center; border: 1px solid #E5E7EB;">
                    <div style="font-size: 12px; color: #6B7280; font-weight: 600; text-transform: uppercase;">${t.totalInferences}</div>
                    <div style="font-size: 22px; font-weight: 700; color: #16A34A; margin-top: 4px;">${stats.totalInferences}</div>
                  </td>
                  <td width="2%"></td>
                  <td width="32%" style="background-color: #F9FAFB; border-radius: 12px; padding: 16px; text-align: center; border: 1px solid #E5E7EB;">
                    <div style="font-size: 12px; color: #6B7280; font-weight: 600; text-transform: uppercase;">${t.totalImages}</div>
                    <div style="font-size: 22px; font-weight: 700; color: #111827; margin-top: 4px;">${stats.totalImages}</div>
                  </td>
                  <td width="2%"></td>
                  <td width="32%" style="background-color: #F9FAFB; border-radius: 12px; padding: 16px; text-align: center; border: 1px solid #E5E7EB;">
                    <div style="font-size: 12px; color: #6B7280; font-weight: 600; text-transform: uppercase;">${t.totalDefects}</div>
                    <div style="font-size: 22px; font-weight: 700; color: #DC2626; margin-top: 4px;">${stats.totalDefects}</div>
                  </td>
                </tr>
              </table>

              ${
                defectsRows
                  ? `
              <!-- Defects Breakdown Table -->
              <h3 style="margin-top: 24px; margin-bottom: 12px; color: #111827; font-size: 15px; font-weight: 600;">
                ${t.defectsBreakdown}
              </h3>
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-bottom: 28px; border-collapse: collapse;">
                ${defectsRows}
              </table>`
                  : ''
              }

              <!-- CTA Button -->
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
                  <td align="center" style="padding-top: 8px; padding-bottom: 8px;">
                    <a href="${downloadUrl}" target="_blank" style="background: linear-gradient(135deg, #16A34A 0%, #15803D 100%); color: #ffffff; text-decoration: none; font-size: 15px; font-weight: 600; padding: 14px 28px; border-radius: 10px; display: inline-block; box-shadow: 0 4px 12px rgba(22, 163, 74, 0.35);">
                      ${t.downloadButton}
                    </a>
                  </td>
                </tr>
              </table>

            </td>
          </tr>

          <!-- Footer -->
          <tr>
            <td style="background-color: #F9FAFB; padding: 20px 32px; text-align: center; border-top: 1px solid #E5E7EB;">
              <p style="margin: 0; color: #6B7280; font-size: 12px; line-height: 1.5;">
                ${t.footerNotice}
              </p>
            </td>
          </tr>

        </table>
      </td>
    </tr>
  </table>
</body>
</html>
  `;
}
